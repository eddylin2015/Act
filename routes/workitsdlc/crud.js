...
router.get('/', require('connect-ensure-login').ensureLoggedIn(), (req, res, next) => {
    ...
    res.render('workitsdlc/list.pug', {
      profile: req.user ,
      books: entities,
...
router.get('/searchform', (req, res) => {
    res.render('workitsdlc/searchform.pug', {
        profile: req.user,
        book: {
...
router.post('/searchform', require('connect-ensure-login').ensureLoggedIn(),
    ...
    getModel().listTimestampBy(
        ...
        (err, entities, cursor, apiResponse) => {
            ...
            res.render('workitsdlc/table.pug', {
                profile: req.user,
                books: entities,
                nextPageToken: cursor
            });
...
router.get('/searchforKW', (req, res) => {
    ...
    res.render('workitsdlc/searchforKW.pug', {
        profile: req.user,
        book: {
            kw: "",
            logDate:  netutils.fmt_now(60),
    ...
    (req, res,next) => {
        ...
        getModel().listByKW(
            ...
            (err, entities, cursor, apiResponse) => {
                ...
                res.render('workitsdlc/list.pug', {  //table.pug
                    profile: req.user,
                    books: entities,
                    nextPageToken: cursor
                });
...
router.get('/add', (req, res) => {
    res.render('workitsdlc/form.pug', {
        profile: req.user,
        book: {
            author: req.user.username,
...
router.get('/addtext', (req, res) => {
    res.render('workitsdlc/formPlain.pug', {
        profile: req.user,
        book: {
            author: req.user.username,
            authorname: req.user.displayName,
...
router.get('/:book', (req, res, next) => {
  getModel().read(req.user.id, req.params.book, (err, entity) => {
    ...
    res.render('workitsdlc/view.pug', {
      profile: req.user,
      book: entity
    });
  });
